...
int  main(int argc, char *argv[])  
{
	...
	if ( myrank == 0) 
	{
		#pragma omp parallel num_threads(NUMTHREADS) private(total_threads, thread_number, msgtag)
		{
			...
			// a thread T do processo mestre recebe as mesangens das threads T dos processos
			// escravos. Por isso src varia de 1 até npes-1 (i.e., src<npes).
			// essa associacao das threads de mesmo nr eh garantida pela msgtag
			// observe que sao recebidas NUMTHREADS msgs de cada um dos npes-1 processos,
	...
	else 
	{
		...
		{
			//(npes-1)*NUMTHREADS threads executam este codigo, sendo
			// NUMTHREADS threads para cada processo MPI
